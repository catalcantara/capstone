...
def tomar_foto(cap, n, modelo, pos, archivo):
    ret, frame = cap.read()
    ruta_im = "./capstone/Procesamiento/Resultados/Con_Fallas/img_" + str(n) + ".jpg"
    resultados = evaluar_fisura (frame, modelo, ruta_im)
    if ret and resultados[0]:
        cv2.imshow("Camara", frame)
        almacenar_info (pos, "F", archivo)

def cargar_pic (ruta, n, modelo, pos, archivo):
...
def evaluar_fisura(frame, modelo, ruta_im, umbral_conf=0.50):
    """
...
def main (num):
    ...
    modelo = YOLO("/capstone/Procesamiento/best.pt")   # <= AJUSTA la ruta a donde guardes el modelo
    ...
    while True:
        ...
        if ser.in_waiting > 0:
            ...
            if capturar:
                ...
                det = tomar_foto (cap, foto_error, modelo, posicion, ruta)
                fotos += 1
...
def main_temporal (num):
    ...
    while True:
        ...
        det = cargar_pic (ruta_final, foto_error, modelo, posicion, ruta_tex)
        fotos += 1
        if det == True: foto_error += 1
