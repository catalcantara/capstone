...
senal_vel = (rpm_min * max_vel / max_vel_rpm) * 5 # Señal proporcional a la velocidad (6.08)
...
def tarea():
    ...
    err_prev = int (0)
    error = int ((cant_enc_d_c - enc_count) * d_c / dt)
    ...
    while True:
        ...
        if hall.value() == 0:
            ...
            print (f"Entró pero son enc_count {enc_count} y cant_enc_d_c {cant_enc_d_c}")
            set_duty(vel_rot * dir_rot + srv_stop, servo_pwm)
            while enc_count < cant_enc_d_c and vel_por < int (100):
                err_prev = error
                ...
                vel = c_prop + c_int #+ c_der
                vel_rpm = (vel / rec_r)
                vel_por = vel_rpm * max_vel / max_vel_rpm
                print (f"velocidad {vel_por}")
